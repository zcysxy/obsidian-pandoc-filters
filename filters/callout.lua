...
function BlockQuote(elem)
  local callout_type = pandoc.utils.stringify(elem.content[1].content[1])
  ...
  if not callout_type then
	...
	-- Special callout types
	local block_color
	if has_value({ "rmk", "remark" }, callout_type) then
		block_color = "red!80!white"
		callout_type = "Remark"
	elseif has_value({ "qn", "question" }, callout_type) then
		block_color = "orange!80!white"
		callout_type = "Question"
	elseif has_value({ "impt", "important", "def", "definition" }, callout_type) then
		block_color = "yellow!80!white"
		callout_type = "Important"
	elseif has_value({ "ex", "example" }, callout_type) then
		block_color = "green"
	...
	-- Get callout title
	local callout_title = pandoc.utils.stringify(get_raw_tex(elem.content[1]))
	callout_title = callout_title:gsub("^%[%!([a-z0-9%-]*)%][%+%-]? ?", "")
	local title, id = callout_title:match("(.*) *^([0-9a-zA-Z-]*)$")
	if title then
		callout_title = title
	end

	table.remove(elem.content, 1)
	local title_only = next(elem.content) == nil

	-- Special callout types
	local latex_begin_string, latex_end_string, latex_begin_string_template
